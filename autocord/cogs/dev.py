...
class Development(commands.Cog):
    ...
    async def reload(self, ctx, *, extension=None):
        ...
        if extension is None:
            ...
            reload = ScrollingPaginator(title=f'Reloaded `{len(reloaded)}` '
                                              f'cog{"s" if len(reloaded) != 1 else ""} {exc}',
                                        entries=entries, per_page=1)
    ...
    async def pull(self, ctx, hard: bool = False):
        ...
        errored = ('fatal', 'error')
        embed = discord.Embed(title=f'GitHub{" Hard" if hard else ""} Pull', description='',
